...
class My_Anime(db.Model):                                                                                                            # creating a My_Anime class  inheriting from db.model
    ...
    def date_filter(cls, year, month, day, user_id):
        sql_query = text("SELECT * FROM  My_Anime WHERE DATE(anime_started) = ':year-:month-:day' and client_id_fk=':user_id';")
        sql_query = text("SELECT * FROM  My_Anime WHERE DATE(anime_finished) = ':year-:month-:day' and client_id_fk=':user_id';")
