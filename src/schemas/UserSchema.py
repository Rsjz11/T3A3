...
from marshmallow.validate import Length, Email                          # Import the length class that will allow us to validate the length of the email
...
class UserSchema(ma.SQLAlchemyAutoSchema):                              # Generates Schema automatically
    ...
    email = ma.String(required=True, validate=[Length(min=4), Email()]) # The email is required and must be at least 4 chars long
    password = ma.String(required=True, validate=Length(min=6))         # The password is required and must be at least 6 chars long
